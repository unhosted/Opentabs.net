  ...
  function (platform) {
    ...
    function userAddress2hostMetas(userAddress, cb) {
      if(typeof(userAddress) != 'string') {
        cb('user address is not a string');
        return;
      }
      ...
      if(parts.length < 2) {
        ...
        if(!(/^[\.0-9A-Za-z]+$/.test(parts[0]))) {
          ...
          cb(null, [
            ...
            //'https://'+parts[1]+'/.well-known/host-meta?resource='+encodeURIComponent('acct:'+userAddress),
            //'http://'+parts[1]+'/.well-known/host-meta.json',
            'http://'+parts[1]+'/.well-known/host-meta?resource='+encodeURIComponent('acct:'+userAddress)
            ]);
        }
      }
    }
          ...
          fetchXrd(hostMetaAddresses, options.timeout, function(err2, hostMetaLinks) {
            if(err2) {
              ...
              if(hostMetaLinks['lrdd'] && hostMetaLinks['lrdd'].template) {
                ...
                for(var i=0; i<hostMetaLinks.length; i++) {
                  if(hostMetaLinks[i].rel=='http://www.w3.org/community/unhosted/wiki/rww-2012.04#simple') {
                    cb(null, hostMetaLinks[i]);
                    return;
                  }
                }
